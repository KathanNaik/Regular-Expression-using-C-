...
public:
    ...
    */
    ...
    bool simulator(string dfa_tab[][3], string word, int state)
    {
        int len = word.length();
...
int main()
{
    ...
    for (int x = 0; x < N; x++)
    {
        ...
        // for (int i = 0; i < 1000 * col; i++)
        // {
        //     nfa_table[i % 1000][i / 1000] = 0;
        // }

        A[x]->initialise(nfa_table);
        int states = 0;

        // Its not able to read regex cleanly

        // for (int i = 0; i < regex.size();)
        // add a NULL char in the end
        //  cin>>s;
        ...
        for (int i = s.size() - 1; i >= 0; i--)
        {
            if (s[i] == '+')
            {
                ...
                for (int j = i - 1; j >= 0; j--)
                {
                    ...
                    if (br == 0)
                    {
                        // insert from j to i in i-1
                        s.insert(j - 1, s.substr(j, i - j));
                        i = j;
                        break;
                        // j->i
                    }
        ...
        states = A[x]->reg_nfa(s, nfa_table);
        A[x]->print_nfa_table(nfa_table, states);

        dfa_state[x] = 0;
        dfa_state[x] = A[x]->nfa_dfa(nfa_table, states, dfa_tab[x]);
        A[x]->print_dfa_table(dfa_tab[x], dfa_state[x]);
    }
    ...
    while (true)
    {
        ...
        char *finale = new char[2 * (word).size()];
        ...
        for (int i = 0; i < (word).size(); i++)
        {
            for (int j = (word).size() - 1; j >= i; j--)
            {
                ...
                for (int x = 0; x < N; x++)
                {
                    bool res = A[x]->simulator(dfa_tab[x], word.substr(i, j - i + 1), dfa_state[x]);
                    cout << i << "-" << j << " "
                         << "->" << x << "\t" << word.substr(i, j - i + 1) << endl;
                    if (res == true)
                    {
                        // cout << j;
                        finale[index++] = '$';
                        finale[index++] = to_string(x + 1)[0];
                        // for (int k = i; k <= j; k++)
                        // {
                        //     finale[index++] = word[k];
                        // }
        ...
        // char *final = new char[2 * word.size()];
        // for (int i = 0; i < 2 * word.size(); i++)
        // {
        //     final[i] = reached ? ' ' : finale[i];
        //     if (finale[i] == '#')
        //     {
        //         reached = true;
        //     }
        ...
        // bool ans = simulator(dfa_tab, word, dfa_state);
